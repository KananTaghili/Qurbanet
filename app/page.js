...
const SERVICES = [
  ...
  },
  {
    ...
    href: "#",
    disabled: true,
    icon: <IconMeat cls="w-10 h-10 text-red-600" />,
  ...
  return (
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
      <div className="px-4">
          ...
          <Link href="/" className="flex items-center gap-1.5 cursor-pointer">
            ...
            <div className="leading-none">
              <div className="font-extrabold text-base text-gray-900 tracking-tight">
                MEAT<span className="text-red-600">BOX</span>.AZ
              </div>
        ...
        {open && (
          <div className="border-t border-gray-100 py-3 flex flex-col gap-3 text-sm font-medium">
            {navLinks.map((l) => (
              <a key={l} href="#" onClick={() => setOpen(false)} className="text-gray-700 hover:text-green-700 py-1 px-1">{l}</a>
            ))}
            <div className="flex items-center gap-4 pt-2 border-t border-gray-100 px-1">
              <Link href="/auth/login" className="flex items-center gap-1.5 text-gray-700">
      ...
      {SERVICES.map((s) => {
        const inner = (
          <div key={s.id} className={`relative rounded-2xl shadow-md overflow-hidden h-44 ${s.thumbGradient}`}>
            ...
            {/* Star-burst badge — top right */}
            <div className="absolute z-20 top-2 right-3">
              <ServiceBadge icon={s.badgeIcon} badgeColor={s.badgeColor} />
            </div>
  ...
  return (
    <section className="relative h-[300px] md:h-[340px] flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 w-full flex flex-col items-center text-center">
        ...
        <p className="text-gray-700 text-xl md:text-2xl mb-6 font-medium">
          Qurbanlıq &bull; Xeyriyyə &bull; Təzə Ət
        </p>
        ...
        <div className="flex flex-wrap justify-center gap-8 text-lg text-gray-700">
          {[
            {
              icon: (
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-12 h-12 text-green-600 shrink-0">
        ...
        {SERVICES.map((s) => (
          <div key={s.id} className="bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden">
            <div className="px-5 pt-5 pb-3 flex flex-col items-center text-center gap-2">
              <div className={`w-16 h-16 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-sm shrink-0`}>
                {s.icon}
              </div>
            ...
            <div className="px-3" onClick={() => onPlay({ url: s.videoUrl, type: s.videoType })}>
              <VideoThumbnail gradient={s.thumbGradient} />
            </div>
            ...
            <div className="px-5 pt-3 pb-5 flex flex-col gap-3 flex-1">
              ...
              {s.disabled ? (
                <button disabled className="bg-gray-300 text-white font-bold py-2.5 px-5 rounded-lg text-sm w-full flex items-center justify-center gap-2 opacity-50 cursor-default">
                  {s.btnLabel}
        ...
        <div className="flex flex-col md:flex-row md:items-stretch gap-6 md:gap-0 pb-8 border-b border-gray-200">
          <div className="flex flex-col gap-3 md:pr-8 md:flex-1">
            <div className="flex items-center gap-2">
              <Image src="/logo_test.png" alt="MeatBox Logo" width={44} height={44} className="object-contain" />
              <div>
