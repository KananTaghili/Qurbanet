...
function MobileHeader() {
  ...
  const navLinks = [
    { label: "Haqqımızda", emoji: "ℹ️" },
    { label: "Xidmətlər", emoji: "🛒" },
    { label: "Necə işləyir?", emoji: "❓" },
    { label: "Əlaqə", emoji: "📞" },
  ];
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: "linear-gradient(135deg, #f0fdf4 0%, #ffffff 50%, #faf5ff 100%)" }}>
          {/* Top bar */}
          <div className="flex items-center justify-between px-5 h-14 border-b border-gray-100">
            <Link href="/" className="flex items-center gap-1.5" onClick={() => setOpen(false)}>
              <Image src="/logo_test.png" alt="MeatBox Logo" width={26} height={26} className="object-contain" />
              <span className="font-extrabold text-base text-gray-900 tracking-tight">
                MEAT<span className="text-red-700">BOX</span>.AZ
              </span>
            </Link>
            <button onClick={() => setOpen(false)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors">
          ...
          <div className="flex-1 flex flex-col justify-center px-6 gap-2">
            {navLinks.map((item, i) => (
              <a
                ...
                className="flex items-center gap-4 px-5 py-4 rounded-2xl bg-white/70 border border-gray-100 shadow-sm hover:shadow-md hover:border-green-200 hover:bg-green-50/50 transition-all group"
                style={{ animationDelay: `${i * 60}ms` }}
              >
                <span className="text-2xl">{item.emoji}</span>
                <span className="font-semibold text-gray-800 text-base group-hover:text-green-700 transition-colors">{item.label}</span>
          ...
          <div className="px-6 pb-10">
            <Link
              href="/auth/login"
              onClick={() => setOpen(false)}
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl bg-green-700 hover:bg-green-800 text-white font-bold text-sm transition-colors shadow-lg shadow-green-200"
            >
              <User size={16} />
              Daxil ol
            </Link>
  ...
  return (
    <section className="px-3 pt-3 pb-4 flex flex-col gap-3">
      {SERVICES.map((s) => {
        ...
        const inner = (
          <div key={s.id} className="relative rounded-2xl shadow-md overflow-hidden h-56 bg-black">
            ...
            {ytId ? (
              <iframe
                ...
                style={{
                  ...
                  width: "calc(224px * 16 / 9)", height: "224px",
            ...
            <div className="absolute inset-0 z-10" style={{
              background: "linear-gradient(to right, rgba(255,255,255,0.97) 0%, rgba(255,255,255,0.92) 40%, rgba(255,255,255,0.5) 62%, rgba(255,255,255,0) 80%)",
            }} />

            {/* Text — left side */}
            <div className="absolute inset-y-0 left-0 z-20 flex flex-col justify-between p-3 w-[58%]">
              <div>
                <h3 className={`text-[13px] font-bold ${s.titleColor} leading-tight line-clamp-3 mb-1`}>
                  {s.title}
                </h3>
                <p className="text-gray-600 text-[10.5px] leading-relaxed line-clamp-3">
                  {s.desc}
                </p>
              </div>
              <button className={`${s.btnCls} text-white font-bold py-1.5 px-3 rounded-lg transition-colors flex items-center gap-1 text-[11px] self-start whitespace-nowrap opacity-${s.disabled ? "50" : "100"}`}>
                {s.btnLabel}
            ...
            >
              <div className="w-12 h-12 rounded-full bg-black/50 flex items-center justify-center shadow-xl border border-white/20">
            ...
            <div className="absolute z-20 top-2 right-3">
              <div
                className={`w-12 h-12 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-md`}
                style={{ boxShadow: `0 4px 12px ${s.serviceIconColor}30` }}
              >
                <s.ServiceIcon size={26} color={s.serviceIconColor} />
              </div>
            </div>
...
function Hero() {
  return (
    <section className="relative h-[300px] md:h-[340px] flex items-center overflow-hidden">
      ...
      {/* Radial white overlay — center bright */}
      <div className="absolute inset-0" style={{
        background: "radial-gradient(ellipse 60% 90% at 50% 62%, rgba(255,255,255,1) 0%, rgba(255,255,255,1) 22%, rgba(255,255,255,0.85) 45%, rgba(255,255,255,0.35) 65%, rgba(255,255,255,0) 80%)",
      }} />
      {/* Bottom white fade */}
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to bottom, transparent 65%, rgba(255,255,255,0.7) 85%, #ffffff 100%)",
      }} />
          ...
          return (
            <div key={s.id} className="bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden">
              <div className="px-5 pt-3 pb-2 flex flex-col items-center text-center gap-1.5">
                {/* react-icon circle */}
                <div
                  className={`w-14 h-14 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-sm shrink-0`}
                  style={{ boxShadow: `0 4px 16px ${s.serviceIconColor}25` }}
                >
                  <s.ServiceIcon size={30} color={s.serviceIconColor} />
                </div>
                <h3 className={`text-xs font-bold ${s.titleColor} leading-tight text-center line-clamp-2`}>
                  {s.title}
                </h3>
              </div>
