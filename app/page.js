...
function MobileServicesSection({ onPlay }) {
  return (
    <section className="px-3 pt-3 pb-4 flex flex-col gap-3">
      {SERVICES.map((s) => {
        const inner = (
          <div key={s.id} className={`relative rounded-2xl shadow-md overflow-hidden h-44 ${s.thumbGradient}`}>
            ...
            >
              <div className="w-12 h-12 rounded-full bg-black/50 flex items-center justify-center shadow-xl">
          ...
          return (
            <div key={s.id} className="bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden">
              <div className="px-5 pt-3 pb-2 flex flex-col items-center text-center gap-1.5">
                {/* react-icon circle */}
                <div
                  className={`w-20 h-20 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-sm shrink-0`}
                  style={{ boxShadow: `0 4px 16px ${s.serviceIconColor}25` }}
                >
                  <s.ServiceIcon size={40} color={s.serviceIconColor} />
                </div>
