      ...
      {SERVICES.map((s) => {
        ...
        const inner = (
          <div key={s.id} className="relative rounded-2xl shadow-md overflow-hidden bg-black" style={{ height: 320 }}>
            ...
            {/* Text — left side */}
            <div className="absolute inset-y-0 left-0 z-20 flex flex-col justify-between p-5 w-[65%]">
              <div>
                <h3 className={`text-lg font-bold ${s.titleColor} leading-tight line-clamp-3 mb-2`}>
                  {s.title}
                </h3>
                <p className="text-gray-600 text-[13px] leading-relaxed line-clamp-4">
                  {s.desc}
                </p>
              </div>
              <button className={`${s.btnCls} text-white font-bold py-2.5 px-5 rounded-xl transition-colors flex items-center gap-2 text-sm self-start whitespace-nowrap opacity-${s.disabled ? "50" : "100"}`}>
                {s.btnLabel}
            ...
            <div className="absolute z-20 top-3 right-3">
              <div
                className={`w-[72px] h-[72px] rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-md`}
