  ...
  return (
    <section className="px-3 pt-3 pb-4 flex flex-col gap-3">
      {SERVICES.map((s) => {
        ...
        const inner = (
          <div key={s.id} className="relative rounded-2xl shadow-md overflow-hidden bg-black" style={{ height: 280 }}>
            ...
            {ytId ? (
              <iframe
                ...
                style={{
                  ...
                  width: "calc(280px * 16 / 9)", height: "280px",
            ...
            <div className="absolute inset-0 z-10" style={{
              background: "linear-gradient(to right, rgba(255,255,255,0.97) 0%, rgba(255,255,255,0.93) 42%, rgba(255,255,255,0.55) 65%, rgba(255,255,255,0) 82%)",
            }} />

            {/* Text — left side */}
            <div className="absolute inset-y-0 left-0 z-20 flex flex-col justify-between p-4 w-[62%]">
              <div>
                <h3 className={`text-base font-bold ${s.titleColor} leading-tight line-clamp-3 mb-2`}>
                  {s.title}
                </h3>
                <p className="text-gray-600 text-[12px] leading-relaxed line-clamp-4">
                  {s.desc}
                </p>
              </div>
              <button className={`${s.btnCls} text-white font-bold py-2 px-4 rounded-xl transition-colors flex items-center gap-1.5 text-[13px] self-start whitespace-nowrap opacity-${s.disabled ? "50" : "100"}`}>
                {s.btnLabel}
            ...
            <div className="absolute z-20 top-3 right-3">
              <div
                className={`w-16 h-16 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-md`}
