          ...
          return (
            <div key={s.id} className="bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden">
              <div className="px-5 pt-3 pb-2 flex flex-col items-center text-center gap-1.5">
                {/* react-icon circle */}
                <div
                  className={`w-16 h-16 rounded-full border-2 ${s.iconRing} flex items-center justify-center shadow-sm shrink-0`}
                  style={{ boxShadow: `0 4px 16px ${s.serviceIconColor}25` }}
                >
                  <s.ServiceIcon size={34} color={s.serviceIconColor} />
                </div>
                <h3 className={`text-sm font-bold ${s.titleColor} leading-tight text-center line-clamp-2`}>
                  {s.title}
                </h3>
              </div>
