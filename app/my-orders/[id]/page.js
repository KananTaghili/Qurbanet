...
import {
  ChevronRight, ChevronDown, ClipboardList, CheckCircle,
  Scissors, Package, Truck, Star, MapPin, ExternalLink,
  Video, ImageIcon, X, Banknote, FileText, ShoppingBag,
  Download, Play, ChevronLeft, Clock,
} from 'lucide-react';
...
const STATUS_COLOR = {
  ...
  delivering: '#3B82F6', completed: '#22C55E', cancelled: '#9CA3AF',
};
const TIMELINE_STEPS = [
  ...
  { key: 'slaughtering', label: 'Kəsilir',           Icon: RiKnifeLine, stage: 'slaughter'  },
...
const DIST_LABELS = {
  catdirilsin: 'Sizə çatdırılsın', ozun_gotur: 'Özüm götürəcəm', ozum: 'Özüm götürəcəm',
  usaqlar_evi: 'Uşaqlar evi', qocalar_evi: 'Qocalar evi', ehtiyac_sahibleri: 'Ehtiyac sahibləri',
};
const CUT_LABELS = {
...
// ── Custom knife icon ─────────────────────────────────────────────────────────
// ── Accordion ─────────────────────────────────────────────────────────────────
function AccordionCard({ id, openId, setOpenId, Icon, iconBg = 'bg-primary-surface', iconColor = 'text-primary', title, children }) {
  const isOpen = openId === id;
  return (
    <div className="bg-surface rounded-2xl border border-border shadow-card overflow-hidden">
      <button className="w-full flex items-center gap-3 px-4 py-4 cursor-pointer bg-transparent border-0 text-left"
        onClick={() => setOpenId(isOpen ? null : id)}>
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${iconBg}`}>
          <Icon size={20} className={iconColor} />
        </div>
        <span className="flex-1 text-sm font-bold text-text-primary">{title}</span>
        {isOpen ? <ChevronDown size={18} className="text-text-secondary flex-shrink-0" />
...
function InfoRow({ label, value, last }) {
  return (
    <div className={`flex justify-between items-start px-4 py-3 gap-3 ${!last ? 'border-b border-border/60' : ''}`}>
      <span className="text-xs text-text-secondary font-medium shrink-0">{label}</span>
      <span className="text-xs font-bold text-text-primary text-right max-w-[60%]">{value || '—'}</span>
    </div>
  );
}
...
function StepMedia({ items, onOpen, pending }) {
  ...
  const shown   = filter === 'video' ? videos : filter === 'photo' ? photos : items;
    ...
    <div className="mt-2 mb-1">
      ...
      <div className="flex gap-2 mb-2 flex-wrap">
        {[
          ...
          ...(videos.length > 0 ? [{ key: 'video', label: `Video ${videos.length}` }] : []),
          ...(photos.length  > 0 ? [{ key: 'photo', label: `Foto ${photos.length}`  }] : []),
        ].map(tab => (
      ...
      <div className="grid grid-cols-2 gap-2">
        {shown.map((m, i) => {
          ...
          return (
            <button key={i} onClick={() => onOpen(items, globalIdx)}
              ...
              {m.type === 'video' ? (
                <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-primary-surface/60">
                  ...
                  <span className="text-[10px] font-bold text-primary">Video {videos.indexOf(m) + 1}</span>
                </div>
              ) : (
                <>
                  <img src={m.url} alt={`Foto ${photos.indexOf(m) + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
        ...
        <div className="w-full h-full flex items-center justify-center px-16 py-2">
          {item.type === 'video' ? (
            <video
              ...
              style={{ maxWidth: '100%', maxHeight: 'calc(100vh - 140px)', background: '#000' }}
            >
              <source src={item.url} />
            </video>
          ) : (
...
export default function OrderDetailPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      const isSelfPickup = ['ozun_gotur','ozum'].includes(o?.distribution?.type) || o?.selfPickup;
      if (isSelfPickup || o?.cashPickupCode) {
        const cfg = await api.get('/app-config/settings');
        const d   = cfg.data?.data;
        if (isSelfPickup && d?.meatPickupLocation?.address) setMeatPickupLocation(d.meatPickupLocation);
  ...
  const timeline    = order.statusTimeline || TIMELINE_STEPS.map(s => ({
    ...
    <div className="flex flex-col flex-1 bg-bg">
      ...
      <div className="flex-1 overflow-y-auto pb-8">
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-4 flex flex-col gap-3">
          ...
          {/* Hero */}
          <div className="rounded-3xl overflow-hidden shadow-lg" style={{ background: 'var(--primary)' }}>
            <div className="p-5 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-1">Sifariş nömrəsi</p>
                <p className="text-white font-extrabold text-xl font-mono leading-tight mb-3">{orderNum}</p>
                <div className="mb-3"><StatusBadge status={order.status} /></div>
                <p className="text-white/80 text-sm font-semibold">{animalName}</p>
                <p className="text-white text-2xl font-extrabold mt-0.5">{totalAmt} AZN</p>
              </div>
              <div className="w-28 h-28 rounded-2xl overflow-hidden bg-white flex-shrink-0"
                style={{ border: `3px solid ${accentColor}` }}>
                <img src={animalImg} alt={animalName} className="w-full h-full object-cover" />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 items-start">
            ...
            <div className="flex flex-col gap-3 w-full md:flex-1 md:min-w-0">
              ...
              <AccordionCard id="timeline" openId={openId} setOpenId={setOpenId}
                Icon={ClipboardList} title="Sifariş gedişatı">
                <div className="px-4 py-4 flex flex-col">
                  {TIMELINE_STEPS.map((step, i) => {
                    const tItem     = timeline.find(t => t.key === step.key) || {};
                    const done      = tItem.done || (STATUS_ORDER.indexOf(step.key) <= currentIdx);
                        ...
                        <div className="flex items-start gap-3">
                          <div className="flex flex-col items-center flex-shrink-0">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${
                              ...
                              <step.Icon size={16} className={done ? 'text-white' : 'text-text-muted'} />
                            </div>
                            {(!last || hasMedia) && (
                              <div className={`w-0.5 mt-1 rounded-full ${done ? 'bg-primary' : 'bg-border'}`}
                                style={{ minHeight: hasMedia ? 16 : 28, flex: hasMedia ? 'none' : 1, height: hasMedia ? 16 : undefined }} />
                            )}
                          </div>
                          <div className={`flex-1 min-w-0 ${hasMedia ? 'pt-1.5 pb-2' : 'pt-1.5 pb-5'}`}>
                            <p className={`text-sm font-bold ${done ? 'text-text-primary' : 'text-text-muted'}`}>{step.label}</p>
                            {tItem.date ? (
                              <div className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full bg-primary-surface border border-primary/20">
                                <span className="text-[11px] font-bold text-primary">{fmtDate(tItem.date)}</span>
                              </div>
                            ) : !done && (
                              <p className="text-xs text-text-muted mt-0.5">Gözlənilir</p>
                            )}
                          </div>
                        </div>
                        ...
                        {hasMedia && (
                          <div className="flex gap-3 mb-4">
                            ...
                            <div className="flex flex-col items-center flex-shrink-0 w-9">
                              {!last && (
                                <div className={`w-0.5 rounded-full flex-1 min-h-[8px] ${done ? 'bg-primary' : 'bg-border'}`} />
                              )}
                            </div>
                            ...
                            <div className="flex-1 min-w-0 pb-2">
                              <StepMedia
                                ...
                                onOpen={(items, idx) => setGallery({ items, idx })}
                              />
                            </div>
                          </div>
                        )}
              ...
              {showCashCode && (
                <AccordionCard id="cash" openId={openId} setOpenId={setOpenId}
                  Icon={Banknote} title="Yerində ödəniş kodu">
                  <div className="px-4 py-5 text-center">
                    <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mb-2">Mağazada göstərin</p>
                    <p className="text-3xl font-extrabold text-text-primary tracking-[0.15em] font-mono">{order.cashPickupCode}</p>
                    {cashPickupLocation && (
            ...
            <div className="flex flex-col gap-3 w-full md:flex-1 md:min-w-0">
              ...
              <AccordionCard id="details" openId={openId} setOpenId={setOpenId}
                Icon={FileText} title="Sifariş məlumatları">
              ...
              {showMeatPickup && (
                <AccordionCard id="pickup" openId={openId} setOpenId={setOpenId}
                  Icon={ShoppingBag} title="Əti götürmə">
              ...
              {order.deliveryConfirmCode && (
                <AccordionCard id="delcode" openId={openId} setOpenId={setOpenId}
                  Icon={Truck} title="Çatdırılma kodu">
                  <div className="px-4 py-5 text-center">
                    <p className="text-3xl font-extrabold text-primary tracking-[0.15em] font-mono mb-1">{order.deliveryConfirmCode}</p>
              ...
              {showReview && (
                <AccordionCard id="review" openId={openId} setOpenId={setOpenId}
                  Icon={Star} iconBg="bg-amber-50" iconColor="text-amber-500" title="Rəy bildirin">
                  <div className="p-4">
                    ...
                    {!reviewed ? (
                      <>
                        <textarea value={comment} onChange={(e) => setComment(e.target.value)}
                          ...
                          rows={3} className="field-input mb-3 resize-none" />
      ...
      {/* Gallery modal */}
      {gallery && (
        <GalleryModal items={gallery.items} startIdx={gallery.idx} onClose={() => setGallery(null)} />
      )}
    </div>
  );
}
