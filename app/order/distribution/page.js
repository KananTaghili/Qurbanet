...
import {
  ...
  ChevronRight, AlertCircle, CheckCircle2, Phone, Plus, X,
} from "lucide-react";
...
export default function DistributionPage() {
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem(DIST_STATE_KEY, JSON.stringify({ selectedKey, pickedLocation, phones, addressNote }));
    } catch (_) {}
  ...
  const maxPortionSplit = order.animal?.hasPortionSplit ? (Number(order.animal?.maxPortionSplit) || 1) : 1;
  ...
  const handleContinue = () => {
    ...
    const distFees = {};
    Object.keys(optionData).forEach((k) => { distFees[k] = optionData[k]?.fee ?? 0; });
  ...
  const AddressSection = ({ className = "" }) => (
    <Card className={`${className} ${submitAttempted && !addrOk ? "ring-2 ring-red-400 border-transparent" : ""}`}>
          ...
          {phones.map((phone, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col gap-0.5">
                ...
                {isEmpty   && <p className="text-[10px] text-red-500 font-semibold px-1">{t(lang, 'enterPhone')}</p>}
                {isInvalid && <p className="text-[10px] text-red-500 font-semibold px-1">{t(lang, 'validAZPhone')}</p>}
              </div>
            );
          })}
        </div>
        ...
        <div className="flex-1 overflow-y-auto pb-24 lg:pb-6 pt-[124px] lg:pt-0">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3
            ...
            <div className="flex flex-col gap-3">
              <Card className={submitAttempted && !selectionOk ? "ring-2 ring-red-400 border-transparent" : ""}>
                <div className="px-3 py-2 border-b border-border bg-surface-alt/40">
